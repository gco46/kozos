...
static int elf_load_program(struct elf_header *header) {
	...
	for (i = 0; i < header->program_header_num; i++) {  // セグメント単位でループ
		// プログラムヘッダを取得
		phdr = (struct elf_program_header *)((char *)header +
											 header->program_header_offset +
											 header->program_header_size * i);
		
		if (phdr->type != 1) {	// ロード可能なセグメントでなければスキップ
			continue;
		}

		// ELF上の動作アドレス（物理アドレス, PA）にセグメント情報をロード
		// ELF先頭(header)からのオフセットで読み込む先頭アドレスを判断する
		memcpy((char *)phdr->physical_addr, (char *)header + phdr->offset, phdr->file_size);
		// メモリ上サイズ - ファイル上サイズ(=余った領域すべて)を0埋め
		// bss領域では0, ポインタもNULLを初期値とするため
